...
/**
 * This is a modification of the base (ImGuiImplGlfw) class with the intention
 * to fix (in possibly questionable ways) issues encountered with multi-context and multi-window ImGui setups
 */
...
public class ImGuiImplGlfwMulti {
	...
	private final Window eWindow;
	private final ImGuiContext cContext;
	
	public ImGuiImplGlfwMulti(Window window, ImGuiContext context) {
		this.eWindow = window;
	...
	protected void updateMouseData() {
		...
		for (int n = 0; n < platformIO.getViewportsSize(); n++) {
			...
			if(window == 0) {
				viewport.setPlatformHandle(eWindow.getHandle());
				
				if(IS_WINDOWS) {
					viewport.setPlatformHandleRaw(GLFWNativeWin32.glfwGetWin32Window(eWindow.getHandle()));
				}
				
				window = eWindow.getHandle();
			}
	...
	protected void updateMouseCursor() {
		...
		for (int n = 0; n < platformIO.getViewportsSize(); n++) {
			...
			if(windowPtr == 0) {
				platformIO.getViewports(n).setPlatformHandle(eWindow.getHandle());
				
				if(IS_WINDOWS) {
					platformIO.getViewports(n).setPlatformHandleRaw(GLFWNativeWin32.glfwGetWin32Window(eWindow.getHandle()));
				}
				
				windowPtr = eWindow.getHandle();
			}
	...
	private final class CreateWindowFunction extends ImPlatformFuncViewport {
		...
		public void accept(final ImGuiViewport vp) {
			...
			glfwSetWindowFocusCallback(vd.window, ImGuiImplGlfwMulti.this::windowFocusCallback);
			glfwSetCursorEnterCallback(vd.window, ImGuiImplGlfwMulti.this::cursorEnterCallback);
			glfwSetCursorPosCallback(vd.window, ImGuiImplGlfwMulti.this::cursorPosCallback);
			glfwSetMouseButtonCallback(vd.window, ImGuiImplGlfwMulti.this::mouseButtonCallback);
			glfwSetScrollCallback(vd.window, ImGuiImplGlfwMulti.this::scrollCallback);
			glfwSetKeyCallback(vd.window, ImGuiImplGlfwMulti.this::keyCallback);
			glfwSetCharCallback(vd.window, ImGuiImplGlfwMulti.this::charCallback);
			glfwSetWindowCloseCallback(vd.window, ImGuiImplGlfwMulti.this::windowCloseCallback);
			glfwSetWindowPosCallback(vd.window, ImGuiImplGlfwMulti.this::windowPosCallback);
			glfwSetWindowSizeCallback(vd.window, ImGuiImplGlfwMulti.this::windowSizeCallback);
	...
	private static final class ShowWindowFunction extends ImPlatformFuncViewport {
		...
		public ShowWindowFunction(long window) {
			super();
			this.window = window;
		}
		...
		public void accept(final ImGuiViewport vp) {
			...
			if (IS_WINDOWS && vp.hasFlags(ImGuiViewportFlags.NoTaskBarIcon)) {
				if(vp.getPlatformHandleRaw() == 0) {
					vp.setPlatformHandleRaw(GLFWNativeWin32.glfwGetWin32Window(window));
				}
	...
	protected void initPlatformInterface() {
		...
		platformIO.setPlatformShowWindow(new ShowWindowFunction(eWindow.getHandle()));
